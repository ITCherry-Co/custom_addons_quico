...
class OdooApiTesting(http.Controller): 
    ...
    def get_categories(self):
        headers = {'Content-Type': 'application/json'}
        try:
            category_records = http.request.env['product.category'].sudo().search([
                '&', ("id", "!=", 1), ('id', '!=', 89)], order='priority asc')

            categories = []
            for cat in category_records:
                if cat.parent_id.id is False and cat.product_count != 0:
                    image_hash = hashlib.md5(cat.image).hexdigest()
                    if image_hash in self.image_cache:
                        image_url = self.image_cache[image_hash]
                    else:
                        image_url = f'https://quico-odoo.net/web/image/product.category/{cat.id}/image/{image_hash}'
            ...
            for category in top_level_categories:
                if (category.image):
                    ...
                    if image_hash in self.image_cache:
                        ...
                        image_url = f'https://quico-odoo.net/web/image/product.category/{category.id}/image/{image_hash}'
                        self.image_cache[image_hash] = image_url
